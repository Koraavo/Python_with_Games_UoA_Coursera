...
# IMPROVEMENT MADE: THE PASSWORD IS RANDOM
# THE PASSWORD_LIST IS 13 OUT OF 15
# made sure there is no index error if the password is smaller than the guess, with try and except
...
def display_password_list(window, location):
    ...
    password_list = ['The Godfather'.upper(), 'The Prestige'.upper(), 'Casablanca'.upper(), 'Coco'.upper(),
                     'Memento'.upper(), 'Shawshank Redemption'.upper(), 'Forrest Gump'.upper(),
                    'The Lives of Others'.upper(), 'Saving Private Ryan'.upper(),
                     'A Clockwork Orange'.upper(), 'Inglourious Basterds'.upper(), 'Vertigo'.upper(),
                     'Interstellar'.upper(), 'The Silence of the Lambs'.upper(), 'The Lion King'.upper(),
                     'whiplash'.upper(), 'amelie'.upper(), 'The Secret in Their Eyes'.upper(), 'V for Vendetta'.upper(),
                     'Andhadhun'.upper(), 'Before Sunrise'.upper(), 'swades'.upper()]

    embedded_size = 40

    # creating a random password list from the list of passwords
    new_list = random.sample(password_list, k = 13)
...
def embed_password(password, size):
    ...
    for index in range(0, split_index):
        embedding = embedding + random.choice(fill)
...
def display_hint(window, password, guess, location):
    ...
    # try and except to avoid index error if the length of the password is smaller than the guesses
